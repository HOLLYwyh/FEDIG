...
import numpy as np
import pandas as pd
import FEDIG_img_Quantity

sys.path.append('..')
from keras.models import load_model

# log
logging.basicConfig(filename='Quantity_ff.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
file_path = 'logfile/RQs/RQ4_Quantity.csv'
try:
...
ca_logger = FEDIG_img_Quantity.cnn_idi_generation(ca_file_names, ca_model, ca_image_path, (256, 256))
df = df.append(
...
df = df.append(pd.DataFrame({'data': ['ff'], 'all_num': ['-'], 'all_nd_num': ['-'],
                             ...
                             'l_num': ['-'], 'l_nd_num': ['-'], 'time': ['-']},
                            index=[0]), ignore_index=True)
ff_logger = FEDIG_img_Quantity.cnn_idi_generation(ff_file_names, ff_model, ff_image_path, (224, 224))
df = df.append(
